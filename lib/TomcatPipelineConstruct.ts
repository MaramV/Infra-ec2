...
import { TomcatService } from './model/TomcatServices';
...
export class PipelineConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: TomcatAsgProps) {
    ...
    const application = new codedeploy.ServerApplication(this, 'CodeDeployApplication', {
      applicationName: props.projectName !== '' ? `${props.projectName}-${props.tomcatServices.name}-application` : `${props.tomcatServices.name}-application`, // optional property
    });

    const pipelinenotifications = ssm.StringParameter.fromStringParameterAttributes(this, `GetPipelineNotifications-${props.tomcatServices.name}`, {
      parameterName: `/${props.account.environment}/pipelinenotifications/arn`,
    }).stringValue;

    props.tomcatServices.services.forEach((service:TomcatService) => {
      const deploymentGroup = new codedeploy.ServerDeploymentGroup(this, `DeploymentGroup-${service.name}`, {
        loadBalancer: codedeploy.LoadBalancer.application(service.targetGroup),//codedeploy.LoadBalancer.application(elbv2.ApplicationTargetGroup.fromTargetGroupAttributes(this,`dg-tg-${service.name}`,{targetGroupArn:service.targetGroup})),
        autoScalingGroups: [service.autoScalingGroup],//[service.asg],
        deploymentGroupName: `${props.projectName !== "" ? props.projectName + "-" : ""}${props.tomcatServices.name}-${service.name}`,
      ...
      const pipeline = new codepipeline.Pipeline(this, `dg-pl-${service.name}`, {
        pipelineName:`${props.projectName !== "" ? props.projectName + "-" : ""}${props.tomcatServices.name}-${service.name}`,
      ...
      sourceStage.addAction(new codepipeline_actions.S3SourceAction({
        actionName: 'S3Source',
        bucketKey: `${props.projectName !== "" ? props.projectName + "/" : ""}${service.contextPath}.zip`,
      ...
      // Notifications: https://docs.aws.amazon.com/dtconsole/latest/userguide/concepts.html
